...
function battleCardToGrave(owner, card) {
  ...
  if (st0 && oi !== -1) {
    ...
    for (const e of staticClauses(card)) {
      const wD = String(e.what || '').toLowerCase();
      if (e.action === 'prevent' && ['destroy', 'leavebyoppspell', 'all', 'otherowncreature'].includes(wD)) {
...
function effectivePowerInner(state, ownerIdx, card, attacking) {
  ...
  // a creature copying another's stats takes that power outright
  const copied = copiedStatsFrom(state, ownerIdx, card);
  if (copied && copied.key !== card.key) {
    const cp = effectivePower(state, ownerIdx, copied, false);
    if (cp != null) p = cp;
  }
...
function breakerCount(state, ownerIdx, card) {
  ...
  const capN = cap ? parseInt(String(cap).replace(/[^0-9]/g, ''), 10) : null;
  const clamp = (n) => (Number.isFinite(capN) ? Math.min(n, capN) : n);
  if (m.tripleBreaker || hasKw(kw, 'triplebreaker')) return clamp(3 + extra);
  if (m.doubleBreaker || hasKw(kw, 'doublebreaker')) return clamp(2 + extra);
  if (extra) return clamp(1 + extra);
...
function hasLegalBlocker(state, defender, atkCard) {
  ...
  return defender.battlezone.some(c => {
    ...
    if (staticClauses(c).some(e2 => e2.action === 'prevent' &&
        ['selfblock', 'block', 'all'].includes(String(e2.what || '').toLowerCase()))) return false;
    ...
    // the attacker may simply forbid blocking outright
    if (hasKw(akw, 'preventoppblock')) return false;
...
function breakOneShield(state, atkIdx, defIdx, attacker, shieldKey, logs, onTrigger) {
  ...
  if (attacker) fireBoardWide(state, 'onowncreaturebreak', logs,
    { onlySide: atkIdx, exceptKey: attacker.key,
      event: { cardId: attacker.id, key: attacker.key, ownerIdx: atkIdx } });
...
function resolveBattle(state, aIdx, aCard, dIdx, dCard, logs) {
  ...
  const aWon = !losers.some(l => l[1].key === aCard.key);
  const dWon = !losers.some(l => l[1].key === dCard.key);
  if (aWon) {
    firePar(state, aIdx, aCard, 'onbattlewin', logs);
    fireBoardWide(state, 'onowncreaturebattlewin', logs,
      { onlySide: aIdx, exceptKey: aCard.key, event: { cardId: aCard.id, key: aCard.key, ownerIdx: aIdx } });
  }
  if (dWon) {
    firePar(state, dIdx, dCard, 'onbattlewin', logs);
    fireBoardWide(state, 'onowncreaturebattlewin', logs,
      { onlySide: dIdx, exceptKey: dCard.key, event: { cardId: dCard.id, key: dCard.key, ownerIdx: dIdx } });
  }
...
const HANDLED_PREVENTS = new Set([
  ...
  'block', 'selfblock', 'anyattack', 'all', 'selfuntap', 'leavebyoppspell',
  'otherowncreature'
]);
...
function creatureDestroyed(owner, _opp, card, logs, wentToGrave) {
  ...
  if (owner.__state) {
    const st = owner.__state;
    const ownIdx = st.players.indexOf(owner);
    // Strike Back: a matching card in hand may now be played for free
    for (const sb of strikeBackWatchers(st, ownIdx, card.id)) {
      owner.pendingShieldTriggers = owner.pendingShieldTriggers || [];
      if (!owner.pendingShieldTriggers.includes(sb.key)) {
        owner.pendingShieldTriggers.push(sb.key);
        if (logs) logs.push(cardLabel(sb.id) + ' may be played for free (Strike Back).');
      }
    }
...
function blockPrevented(state, attackerIdx, attacker, blocker) {
  ...
  for (const e of staticClauses(attacker)) {
    if (e.action !== 'prevent') continue;
    const w0 = String(e.what || '').toLowerCase();
    if (w0 !== 'oppblock' && w0 !== 'block' && w0 !== 'all') continue;
    if (!e.cardFilter || cardMatchesFilter(blocker.id, e.cardFilter)) return cardLabel(attacker.id);
  }
  return null;
}
...
// "copyStats": this creature takes on the power (and breaker count) of another.
function copiedStatsFrom(state, ownerIdx, card) {
  const kw = grantedKeywords(state, ownerIdx, card);
  if (!hasKw(kw, 'copystats')) return null;
  const spec = kwArg(kw, 'copystats');
  const me = state.players[ownerIdx];
  // default: the creature it is crossed to, else the strongest other creature you have
  const gear = (me.crossGear || []).find(g => g.key === card.key);
  if (gear && gear.crossedTo) return me.battlezone.find(c => c.key === gear.crossedTo) || null;
  const others = me.battlezone.filter(c => c.key !== card.key && !isSpellCard(c.id));
  if (!others.length) return null;
  if (spec) {
    const want = String(spec).replace(/^race=/i, '');
    const m = others.find(c => raceMatchesAny(c.id, want));
    if (m) return m;
  }
  return others.sort((a, b) => (powerOf(b.id) || 0) - (powerOf(a.id) || 0))[0];
}

// Strike Back: while this is in hand, the destruction of a matching creature lets it
// be played for free. Recorded on the state so the free-play offer can be made.
function strikeBackWatchers(state, ownerIdx, destroyedId) {
  const me = state.players[ownerIdx];
  const out = [];
  for (const c of me.hand) {
    const kw = selfGrantedKeywords(c.id);
    if (!hasKw(kw, 'strikeback')) continue;
    const spec = kwArg(kw, 'strikeback');
    if (spec) {
      const want = String(spec).replace(/^civ=/i, '');
      if (!civMatches(destroyedId, want)) continue;
    }
    out.push(c);
  }
  return out;
...
// Cyclone: the creature summoned immediately before this one returns to hand at the
// end of the turn, in exchange for the effect.
function applyCyclone(state, ownerIdx, logs) {
  const me = state.players[ownerIdx];
  const prev = me.lastSummonedKey ? me.battlezone.find(c => c.key === me.lastSummonedKey) : null;
  if (!prev) return;
  prev.tempGrants = prev.tempGrants || [];
  prev.tempGrants.push({ keyword: 'cyclone', arg: null });
  if (logs) logs.push(cardLabel(prev.id) + ' will return to hand at end of turn (Cyclone).');
}

// "protectedFrom[...]" and "immuneToOppSpellRemoval": this creature can't be chosen
// or removed by the described effects.
function protectedFromEffect(state, ownerIdx, card, byOpponent, viaSpell) {
  const kw = grantedKeywords(state, ownerIdx, card);
  if (byOpponent && viaSpell && hasKw(kw, 'immunetooppspellremoval')) return true;
  if (!hasKw(kw, 'protectedfrom')) return false;
  const spec = String(kwArg(kw, 'protectedfrom') || '').toLowerCase();
  if (!spec) return byOpponent;
  if (/spell/.test(spec) && viaSpell) return true;
  if (/opp/.test(spec) && byOpponent) return true;
  return false;
}
...
// "mustBeAttacked": while it is out, other creatures cannot be chosen as the target.
function attackMagnetFor(state, victimIdx) {
  for (const c of state.players[victimIdx].battlezone) {
    if (hasKw(grantedKeywords(state, victimIdx, c), 'mustbeattacked')) return c;
  }
  return null;
}

function attackTargetForbidden(state, victimIdx, victim, attacker) {
  const magnet = attackMagnetFor(state, victimIdx);
  if (magnet && magnet.key !== victim.key) {
    return cardLabel(magnet.id) + ' must be attacked first';
  }
...
function shieldTriggersSuppressed(state, defenderIdx) {
  // a keyword form of the same lock
  for (let si = 0; si < state.players.length; si++) {
    if (si === defenderIdx) continue;
    for (const c of state.players[si].battlezone) {
      if (hasKw(grantedKeywords(state, si, c), 'negateshieldtrigger')) return cardLabel(c.id);
    }
  }
  for (let si = 0; si < state.players.length; si++) {
    for (const src of state.players[si].battlezone) {
  ...
  for (let si = 0; si < state.players.length; si++) {
    for (const src of state.players[si].battlezone) {
      for (const e of staticClauses(src)) {
        if (e.action !== 'prevent') continue;
        const w = String(e.what || '').toLowerCase();
        if (w !== 'untap' && w !== 'untappermana' && w !== 'selfuntap') continue;
        // "selfUntap" restrains the card's own controller; the others restrain the opponent
        if (w === 'selfuntap') { if (si !== playerIdx) continue; }
        else if (si === playerIdx) continue;
...
function attackerForbidden(state, ownerIdx, attacker) {
  // a blanket "this creature cannot attack" keyword
  if (hasKw(grantedKeywords(state, ownerIdx, attacker), 'cannotattack')) {
    return cardLabel(attacker.id) + ' cannot attack';
  }
  ...
  for (const e of staticClauses(attacker)) {
    if (e.action !== 'prevent') continue;
    const w = String(e.what || '').toLowerCase();
    if (w !== 'selfattack' && w !== 'attack' && w !== 'anyattack' && w !== 'all') continue;
    ...
    switch (e.action) {
        ...
        if (e.selector && e.selector.attachedToSelf) {
          ...
          for (const g of mine.slice()) {
            ...
            if (act === 'toGrave' || act === 'destroy') {
              ...
              fireBoardWide(state, 'oncrossgeargraveyard', logs,
                { event: { cardId: g.id, key: g.key, ownerIdx: meIdx } });
      ...
      case 'evoCharge': {
        ...
        me.pendingTargets.push({
          ...
          // The spell is deferred until this prompt is answered, so it must carry its
          // own key — otherwise nothing retires it and it sits in the battle zone.
          spellKey: isSpellCard(cardId) ? cardKey : null
        });
        defer = true;
        break;
      ...
      case 'freeCast': {
        ...
        logs.push(cardLabel(cardId) + ' was cast for free.');
        break;
      }
      case 'shuffleDeck': {
      ...
      case 'uncross': {
        // strip gear off creatures entirely — it stays in play, just unattached
        const targetsU = (e.selector && e.selector.side === 'opp') ? [opp]
                       : (e.selector && e.selector.side === 'own') ? [me] : [me, opp];
        let n = 0;
        for (const pl of targetsU) for (const g of (pl.crossGear || [])) {
          if (g.crossedTo) { g.crossedTo = null; n++; }
        }
        logs.push(cardLabel(cardId) + ': uncrossed ' + n + ' Cross Gear.');
        break;
      }
      case 'reorderTop': {
        const n = typeof e.count === 'number' ? e.count : 1;
        logs.push('looked at the top ' + n + ' cards and rearranged them.');
        break;
      }
      case 'revealTop': {
        let n = 1;
        if (typeof e.count === 'number') n = e.count;
        else if (e.count && e.count.dynamic) {
          n = countSelector(state, meIdx, { key: cardKey, id: cardId }, e.count.dynamic) * (e.count.times || 1);
        }
        logs.push('revealed the top ' + n + ' card' + (n === 1 ? '' : 's') + ' of their deck.');
        break;
      }
      case 'peekOppDeckTop': {
        logs.push("looked at the top card of their opponent's deck.");
        break;
      }
      case 'forceBattle': {
        const n = typeof e.count === 'number' ? e.count : 1;
        logs.push(cardLabel(cardId) + ': forces ' + n + ' battle(s).');
        break;
      }
      case 'bothDiscard': {
        for (const pl of [opp, me]) {
          if (!pl.hand.length) continue;
          const cnt = e.count === 'all' ? pl.hand.length : (typeof e.count === 'number' ? e.count : 1);
          pl.pendingDiscards.push({ id: newKey(), kind: e.mode === 'choose' ? 'choose' : 'random',
                                    count: cnt, source: cardLabel(cardId) });
        }
        break;
      }
      case 'choose': {
        // a bare "choose N X" with no destination just marks a selection
        logs.push(cardLabel(cardId) + ': choose a card.');
        break;
      }
      case 'counterSpell': {
        logs.push(cardLabel(cardId) + ': countered the spell.');
        break;
      }
      case 'cancelBattle': {
        state.cancelNextBattle = true;
        logs.push(cardLabel(cardId) + ': the battle is cancelled.');
        break;
      }
      case 'reveal': {
...
function battlefieldSlot(me) {
  // Six columns at 16.2% apart. Seven at 13.8% left the cards touching even at their
  // old size, and overlapping once the battle zone was enlarged — the spacing has to
  // leave room for the widest card the layout uses.
  const cols = 6;
  const slotXY = i => ({ x: 2 + (i % cols) * 16.2, y: Math.min(20, Math.floor(i / cols) * 20) });
  const taken = pos => me.battlezone.some(c =>
    c.x != null && Math.abs(c.x - pos.x) < 5 && Math.abs((c.y == null ? 0 : c.y) - pos.y) < 6);
    ...
    switch (msg.type) {
      case 'drawCard': {
        ...
        if (c) {
          ...
          fireBoardWide(s, 'onoppdraw', extraLogs,
            { onlySide: oppIdx, event: { cardId: c, key: null, ownerIdx: idx } });
      ...
      case 'crossGear': {
        ...
        firePar(s, idx, target, 'oncrossed', extraLogs);
      ...
      case 'endTurn': {
        ...
        // Cyclone: the marked creature returns to its owner's hand
        for (const c of me.battlezone.slice()) {
          if (!hasKw(grantedKeywords(s, idx, c), 'cyclone')) continue;
      ...
      case 'chargeMana': {
        ...
        fireBoardWide(s, 'onownmanaenter', extraLogs,
          { onlySide: idx, event: { cardId: c.id, key: c.key, ownerIdx: idx } });
        break;
      }
      case 'summonCard': {
        const i = me.hand.findIndex(c => c.key === msg.key);
        ...
        if (isCrossGear(cardId)) {
          ...
          fireBoardWide(s, 'onanycrossgearenter', extraLogs,
            { event: { cardId, key: c.key, ownerIdx: idx } });
        ...
        me.prevSummonedKey = me.lastSummonedKey || null;
        me.lastSummonedKey = c.key;
        ...
        if (isSpellCard(c.id)) {
          ...
          fireBoardWide(s, 'onanyspellcast', extraLogs, { event: castEv });
      ...
      case 'castFreeFromHand': {
        ...
        {
          const fc = me.hand.find(h => h.key === msg.key);
          if (fc) fireBoardWide(s, 'onanyfreeplay', extraLogs,
            { event: { cardId: fc.id, key: fc.key, ownerIdx: idx } });
        }
      ...
      case 'battleTap': {
        ...
        if (!c.tapped) {
          ...
          const grantsChoice = hasSheetTrigger(c.id, 'tapability') || hasSheetTrigger(c.id, 'silentskill') ||
            hasKw(grantedKeywords(s, idx, c), 'tapability') || TAP_ABILITIES[name] ||
      ...
      case 'effectTarget': {
        ...
        switch (eff.action) {
          ...
          case 'toOwnerMana': {
            if (owner.battlezone.includes(card)) {
              fireBoardWide(s, 'onowncreaturetomana', extraLogs,
                { onlySide: s.players.indexOf(owner), exceptKey: card.key,
          ...
          case 'toGrave':
            ...
            if (eff.zone === 'ownMana') fireBoardWide(s, 'onownmanatograve', extraLogs, { onlySide: s.players.indexOf(owner) });
      ...
      case 'declareAttack': {
        ...
        fireBoardWide(s, 'onanycreatureattack', extraLogs, { event: atkEv });
      ...
      case 'declareBlock': {
        ...
        if (msg.blockerKey) {
          ...
          fireBoardWide(s, 'onowncreatureblocked', extraLogs,
            { onlySide: cb.attackerIdx, exceptKey: atk.key,
        ...
        logText = 'chose not to block.';
        {
          ...
          if (atkC) {
            firePar(s, cb.attackerIdx, atkC, 'onunblockedattack', extraLogs);
            fireBoardWide(s, 'onoppunblockedattack', extraLogs,
              { onlySide: idx, event: { cardId: atkC.id, key: atkC.key, ownerIdx: cb.attackerIdx } });
          }
        }
