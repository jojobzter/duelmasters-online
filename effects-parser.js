...
const TRIGGERS = new Set([
  ...
  'ongeneratecrossgear',
  // added with this batch
  'onowncreaturebattlewin', 'onanycreatureattack', 'onoppunblockedattack',
  'onanycrossgearenter', 'onselfdiscard', 'oncrossed', 'oncrossgeargraveyard',
...
const ZONES = {
  ...
  // the creature whose arrival/attack fired this trigger
  enteringcreature: { side: 'any', zone: 'event' },
  attacker:         { side: 'any', zone: 'event' },
  attackedcreature: { side: 'any', zone: 'event' },
  actorcreature:    { side: 'any', zone: 'event' },
  source:           { side: 'own', zone: 'event' },
  crossgear:        { side: 'own', zone: 'crossgear' },
  revealed:         { side: 'own', zone: 'revealed' },
  owndecktop:       { side: 'own', zone: 'decktop' },
  owndeck:          { side: 'own', zone: 'deck' },
...
function parseSelector(raw) {
  ...
  let accessor = null;
  const acc = raw.match(/^(.*)\.(topCard|count|civCount|distinctCivs|evoBase|name|crossedCreature|crossedGear|crossed|underCards|owner|wasAdded)$/);
  if (acc) { raw = acc[1]; accessor = acc[2]; }
  // "self.crossedCreature" / "target.crossedCreature" resolve to a crossed creature
  if (accessor === 'underCards' || /\.undercards$/i.test(raw)) {
    return { name: 'self.underCards', side: 'own', zone: 'under', filters: [], accessor: null };
  }
...
function processBody(bodyIn) {
  const mods = {};
  let body = bodyIn;
  body = body.replace(/,\s*(optional|oppChoice|reveal|untilNextTurn|shuffled|noShieldTrigger|loseShieldTrigger|reorder|permanent|tapped|free|noTrigger|useShieldTrigger|instead|actorChoice)\b/gi,
...
function parseClause(raw, cardName, inheritedTrigger) {
  ...
  let m = clause.match(/^resolvesTo\s+(mana|grave|hand|shield)(\s+tapped)?(?:\s+if\s+(.+))?$/i);
  if (m) return { kind: 'property', property: 'resolvesTo', to: m[1].toLowerCase(),
                  tapped: !!m[2], condition: m[3] ? m[3].trim() : null };
  ...
  // "onSummon: resolvesTo hand if ..." — the destination rule, written under a trigger
  const rt = clause.match(/^[a-zA-Z]+\s*:\s*resolvesTo\s+(mana|grave|hand|shield)(\s+tapped)?(?:\s+if\s+(.+))?$/i);
  if (rt) return { kind: 'property', property: 'resolvesTo', to: rt[1].toLowerCase(),
                   tapped: !!rt[2], condition: rt[3] ? rt[3].trim() : null };
...
function parseAction(body, mods) {
  ...
  // "cast: free if ..." waives the mana cost when the condition holds
  if (/^free$/i.test(body.trim())) return { action: 'freeCast' };
  // "target.owner.<action>" — the target's controller performs it, not the caster
  const byOwner = body.match(/^target\.owner\.(.+)$/i);
  if (byOwner) {
    const inner = parseAction(byOwner[1].trim(), mods);
    return inner ? Object.assign({}, inner, { performedBy: 'targetOwner' }) : null;
  }
  ...
  // ", rest -> X" belongs to the clause, not to the action's destination — don't let
  // the generic extractor swallow it.
  const hasRestClause = /,\s*rest\s*->/i.test(rest.join(' '));
  if (arrowAt > -1 && !hasRestClause &&
      !['fromdeck','search','ownkeeps','oppkeeps','keeps'].includes(verb)) {
  ...
  switch (verb) {
    ...
    case 'toshield': case 'todecktop': case 'tap': case 'untap': {
      // "toHand all X, rest -> deckBottom" carries a destination for the leftovers.
      // Check this BEFORE parsing the selector — the raw text still has the tail on it,
      // so the selector parse would fail and bail out first.
      const restArrow = rest.join(' ').match(/^(.*?),\s*rest\s*->\s*(\w+)\s*$/i);
      if (restArrow) {
        const c3 = parseCount(restArrow[1].split(/\s+/));
        const map3 = { tohand: 'toHand', tomana: 'toMana', tograve: 'toGrave',
                       toshield: 'toShield', todecktop: 'toDeckTop', destroy: 'destroy',
                       bounce: 'bounce', tap: 'tap', untap: 'untap' };
        return { action: map3[verb] || verb,
                 count: c3.count, optional: !!c3.optional,
                 selector: parseSelector(c3.rest) || { name: c3.rest },
                 rest: restArrow[2].toLowerCase() };
      }
      const c = parseCount(rest);
    ...
    case 'cancelbattle': return { action: 'cancelBattle', optional: !!(mods && mods.optional) };
    case 'counter': {
      return { action: 'counterSpell', selector: parseSelector(rest[0]) || { name: rest[0] } };
    }
    case 'untap': {
      const c = parseCount(rest);
      const sel = parseSelector(c.rest);
    ...
    case 'keeps': {
      // "keeps 3 ownMana, rest -> grave" — you keep N, the rest go somewhere
      const txt = rest.join(' ');
      const km = txt.match(/^(\S+)\s+([^,]+?)(?:\s*,\s*rest\s*->\s*(\w+))?$/i);
      if (!km) return null;
      return { action: 'ownKeeps',
               keep: /^\d+$/.test(km[1]) ? parseInt(km[1], 10) : { dynamic: km[1] },
               selector: parseSelector(km[2].trim()) || { name: km[2].trim() },
               rest: km[3] ? km[3].toLowerCase() : 'grave' };
    }
    case 'battle': {
      const c = parseCount(rest);
      return { action: 'forceBattle', count: c.count,
               selector: parseSelector(c.rest) || { name: c.rest } };
    }
    case 'bothdiscard': {
      const kindWord = (rest[0] || 'random').toLowerCase();
      const c = parseCount(rest.slice(1));
      return { action: 'bothDiscard', mode: kindWord, count: kindWord === 'all' ? 'all' : c.count };
    }
    case 'reordertop': return { action: 'reorderTop', count: parseInt(rest[0], 10) || 1 };
    case 'revealtop': {
      // may be "revealTop 2x target.count ownDeck"
      const m2 = rest.join(' ').match(/^(\d+)x\s+(\S+)/i);
      if (m2) return { action: 'revealTop', count: { dynamic: m2[2], times: parseInt(m2[1], 10) } };
      return { action: 'revealTop', count: parseInt(rest[0], 10) || 1 };
    }
    case 'peekoppdecktop': return { action: 'peekOppDeckTop' };
    case 'shuffledeck': return { action: 'shuffleDeck' };
