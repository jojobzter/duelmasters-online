...
cards/
  ...
  DM-2/
    ...
  card back/
    back.jpg          <- optional: your card-back art, used for all face-down cards
```

Folder names become the searchable "set". File names (minus extension)
become the card's display name. The special folder **`card back`** (any
capitalization) isn't treated as a set — whatever image is inside it is used
as the universal card-back texture for the deck, opponent's hand, and
face-down shields.

**Each player needs their own copy of the `cards` folder on their own
computer** — images never get uploaded, only card names/positions sync
between players, so folder/file names must match.

---

## 2. Deploy the server (one-time)

Same as before — push the project to a GitHub repo and connect it to a free
Render web service (Build: `npm install`, Start: `npm start`). See the repo
history if you need the detailed click-by-click steps again.
...
Happy to keep extending this — just ask.
